...
public class TestTask extends TestCase {
	...
	public void testWhenAllSuccess() {
		runTaskTest(new Callable<Task<?>>() {
			...
			public Task<?> call() throws Exception {
				...
				for (int i = 0; i < 20; i++) {
					Task<Void> task = Task
							.callInBackground(new Callable<Void>() {
								@Override
				...
				return Task.whenAll(tasks).continueWith(
						new Continuation<Void, Void>() {
							...
							public Void then(Task<Void> task) {
								...
								assertEquals(2, ((AggregateException) task
										.getError()).getErrors().size());
								assertEquals(error, ((AggregateException) task
										.getError()).getErrors().get(0));
								assertEquals(error, ((AggregateException) task
										.getError()).getErrors().get(1));
	...
	public void testOnSuccess() {
		...
		Task<Integer> error = Task.<Integer> forError(
				new IllegalStateException()).onSuccess(continuation);
		Task<Integer> cancelled = Task.<Integer> cancelled().onSuccess(
				continuation);
	...
	public void testOnSuccessTask() {
		...
		Task<Integer> error = Task.<Integer> forError(
				new IllegalStateException()).onSuccessTask(continuation);
		Task<Integer> cancelled = Task.<Integer> cancelled().onSuccessTask(
				continuation);
